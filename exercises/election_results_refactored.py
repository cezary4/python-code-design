...
def download_data():
	'''download data from url and save to filename'''
	urllib.request.urlretrieve(url, filename)

def load_data():
	'''load data from downloaded data, return iterator of dicts'''
	# Create reader for ingesting CSV as array of dicts
	return csv.DictReader(open(filename, 'r', encoding='UTF-8'))
...
def clean_data(raw_data):
	'''given data, iterator of dicts, clean and return defaultdict of data'''
	...
	for row in raw_data:
        ...
        # essentially this: results[race_key].setdefault(cand_key, []).append(row)
	return results
...
def main():
	...
	with open(outfile, 'wb') as fh:
		...
		for race, results in summary.items():
			...
			for cand in cands:
				results.update(cand)
				writer.writerow(bytes(results, 'UTF-8'))
